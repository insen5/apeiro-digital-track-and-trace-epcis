...
###############################################################################
# Scheduled UAT Facility Master Data Sync with Quality Audit (Every 3 Hours)
#
# This script:
# 1. Syncs facilities from Safaricom HIE Facility Registry API
# 2. Saves a quality audit snapshot after successful sync
#
# Installation:
#   crontab -e
#   # Add this line:
#   0 */3 * * * /path/to/scheduled-uat-facility-sync.sh >> ~/logs/uat-facility-sync.log 2>&1
#
...
ENDPOINT="$API_URL/api/master-data/uat-facilities/sync"
LOG_FILE="${LOG_FILE:-$HOME/logs/uat-facility-sync.log}"
...
echo ""
echo "==================================================="
echo "$TIMESTAMP - Starting scheduled UAT facility sync"
echo "==================================================="
...
if [ "$HTTP_STATUS" -eq 200 ] || [ "$HTTP_STATUS" -eq 201 ]; then
  ...
  AUDIT_ENDPOINT="$API_URL/api/master-data/uat-facilities/quality-audit"
  ...
  if [ "$AUDIT_HTTP_STATUS" -eq 200 ] || [ "$AUDIT_HTTP_STATUS" -eq 201 ]; then
    ...
    AUDIT_SCORE=$(echo "$AUDIT_BODY" | jq -r '.overallQualityScore // "N/A"' 2>/dev/null || echo "N/A")
...
echo "==================================================="
echo "$TIMESTAMP - UAT facility sync completed"
echo "==================================================="
echo ""

...
/**
 * Production Facility Entity
 * 
 * Represents healthcare facility master data synced from Safaricom HIE Facility Registry API (Production).
 * Used for track and trace operations in the Kenya TNT system.
 * 
 * Data Source: Safaricom Health Information Exchange (HIE) Facility Registry API - Production
...
/**
 * Production Facilities Sync Log Entity
 * 
 * Tracks sync operations for audit and monitoring purposes.
 */
@Entity('prod_facilities_sync_log')
@Index(['syncStartedAt'])
@Index(['syncStatus'])
export class ProdFacilitiesSyncLog {
  @PrimaryGeneratedColumn()
  id: number;
...
/**
 * Production Facilities Quality Audit Entity
 * 
 * Stores historical data quality metrics for trend analysis.
 */
@Entity('prod_facilities_quality_audit')
@Index(['auditDate'])
export class ProdFacilitiesQualityAudit {
  @PrimaryGeneratedColumn()
  id: number;

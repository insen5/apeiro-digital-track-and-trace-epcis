...
export class EPCISEventService {
  ...
  /**
   * Create Hierarchy Change Event (Pack operation)
   * AggregationEvent with action=ADD for packing cases into package
   */
  async createPackEvent(
    packageSSCC: string,
    caseEPCs: string[],
    options?: {
      actorUserId?: string;
      ...
      bizLocation?: { id: string };
      readPoint?: { id: string };
      packType?: 'PACK' | 'PACK_LITE' | 'PACK_LARGE';
    }
  ): Promise<string> {
    this.logger.log(`Creating Pack event for package ${packageSSCC} with ${caseEPCs.length} cases`);

    return this.createAggregationEvent(
      packageSSCC, // parentId
      caseEPCs, // childEPCs
      {
        action: 'ADD',
  ...
  /**
   * Create Hierarchy Change Event (Unpack operation)
   * AggregationEvent with action=DELETE for unpacking package into cases
   */
  async createUnpackEvent(
    packageSSCC: string,
    caseEPCs: string[],
    options?: {
      actorUserId?: string;
      ...
      bizLocation?: { id: string };
      readPoint?: { id: string };
      unpackType?: 'UNPACK' | 'UNPACK_ALL';
    }
  ): Promise<string> {
    this.logger.log(`Creating Unpack event for package ${packageSSCC} releasing ${caseEPCs.length} cases`);
  ...
  /**
   * Create Repack Event
   * Combination of unpack (DELETE) followed by pack (ADD) with new SSCC
   */
  async createRepackEvent(
    oldSSCC: string,
    newSSCC: string,
    caseEPCs: string[],
    options?: {
  ...
  ): Promise<{ unpackEventId: string; packEventId: string }> {
    this.logger.log(`Creating Repack events: ${oldSSCC} → ${newSSCC}`);

    // First, unpack from old SSCC
    const unpackEventId = await this.createUnpackEvent(oldSSCC, caseEPCs, options);

    // Then, pack into new SSCC
    const packEventId = await this.createPackEvent(newSSCC, caseEPCs, {
      ...options,
      packType: 'PACK', // Repack is just a regular pack after unpack
    });

    return { unpackEventId, packEventId };
  }

  /**
   * Create Return Receiving Event
   * ObjectEvent for receiving returned products
   */
  async createReturnReceivingEvent(
    epcs: string[],
    options?: {
      actorUserId?: string;
      actorType?: string;
    ...
    this.logger.log(`Creating Return Receiving event for ${epcs.length} EPCs`);
    ...
    return this.createObjectEvent(
      epcs,
      {
        ...
        bizTransactionList: options?.referenceDocumentNumber ? [
          {
            type: 'urn:epcglobal:cbv:btt:rma', // Return Merchandise Authorization
            bizTransaction: options.referenceDocumentNumber,
          }
        ] : undefined,
        ...options,
      }
  ...
  /**
   * Create Return Shipping Event
   * ObjectEvent for shipping products back
   */
  async createReturnShippingEvent(
    epcs: string[],
    options?: {
      actorUserId?: string;
      actorType?: string;
    ...
    this.logger.log(`Creating Return Shipping event for ${epcs.length} EPCs`);
    ...
    return this.createObjectEvent(
      epcs,
      {
        ...
        bizTransactionList: options?.referenceDocumentNumber ? [
          {
            type: 'urn:epcglobal:cbv:btt:rma',
            bizTransaction: options.referenceDocumentNumber,
          }
        ] : undefined,
        ...options,
